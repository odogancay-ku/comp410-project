...
void detectAndHandleCollisionSAT(Entity *object1, Entity *object2) {
    ...
    if (!object1->isKinematic && !object2->isKinematic) {
...
//    std::vector<glm::vec3> hitboxVertices2 = object2->modelData->hitboxVertices;
    ...
    for (auto &i: hitboxVertices1) {
        i = glm::vec3(object1->getTransform() * glm::vec4(i, 1.0f));
    }

    for (auto &i: hitboxVertices2) {
        i = glm::vec3(object2->getTransform() * glm::vec4(i, 1.0f));
    }
    ...
    if (calculateCollision(hitboxVertices1, hitboxVertices2, MTV)) {
        ...
        if (!object1->isKinematic) {
            ...
            if (velocityAlongMTV >= 0) {
                return; // Entitys are moving away from each other
            }
